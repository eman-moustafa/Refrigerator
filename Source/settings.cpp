  ...
  intValidator(new QIntValidator(0, 4000000, this))
{
  ...
  connect(ui->serialPortInfoListBox, QOverload<int>::of(&QComboBox::currentIndexChanged),
          this, &Settings::showPortInfo);

  connect(ui->serialPortInfoListBox, QOverload<int>::of(&QComboBox::currentIndexChanged),
          this, &Settings::checkCustomDevicePathPolicy);
...
Settings::Setting Settings::settings() const
{
    return currentSettings;
}

void Settings::showPortInfo(int index)
{
    if (index == -1)
        return;

    const QStringList list = ui->serialPortInfoListBox->itemData(index).toStringList();
...
void Settings::checkCustomDevicePathPolicy(int index)
{
    const bool isCustomPath = !ui->serialPortInfoListBox->itemData(index).isValid();
    ui->serialPortInfoListBox->setEditable(isCustomPath);
    if (isCustomPath)
        ui->serialPortInfoListBox->clearEditText();
}

void Settings::fillPortsParameters()
{
